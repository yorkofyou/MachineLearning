...
def get_adjacent_matrix(distance_file: str) -> np.array:
    ...
    :return:
    """
    A = np.loadtxt(distance_file, delimiter=',')
    return A
...
def get_flow_data(flow_file: str) -> np.array:
    ...
    :return:
    """
    data = np.loadtxt(flow_file, delimiter=',')
    flow_data = data.T[:, :, np.newaxis]
    return flow_data
...
def get_loader():
    num_nodes = 228
    train_data = PEMSDataset(data_path=["../W.csv", "../traffic.txt"], num_nodes=num_nodes,
                             divide_days=[35, 9],
    ...
    test_data = PEMSDataset(data_path=["../W.csv", "../traffic.txt"], num_nodes=num_nodes,
                            divide_days=[35, 9],
                            time_interval=5, history_length=6,
                            train_mode="test")
